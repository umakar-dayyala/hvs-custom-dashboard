...
const API_BASE_URL = `${process.env.REACT_APP_API_PROTOCOL}://${process.env.REACT_APP_API_IP}:${process.env.REACT_APP_API_PORT}/api/fcad`

export const fetchFcadParamChartData = async (deviceId, fromTime, toTime) => {
  try {
    const url = `${API_BASE_URL}/getFcadChart`;
    const params = {
...
export const fetchAnomalyChartData = async (deviceId, fromTime, toTime) => {
  try {
      const url = `${API_BASE_URL}/getFcadChartAnomaly`;
      const params = {
        param_device_id: deviceId,
        // param_device_id: 43,
        param_start_date: `${fromTime}`,  // Add single quotes around the dates
...
export const fetchOutlierChartData = async (deviceId, fromTime, toTime) => {
  try {
      const url = `${API_BASE_URL}/getFcadChartOutlier`;
      const params = {
        param_device_id: deviceId,
        // param_device_id: 43,
        param_start_date: `${fromTime}`,  // Add single quotes around the dates
