...
const API_BASE_URL = `${process.env.REACT_APP_API_PROTOCOL}://${process.env.REACT_APP_API_IP}:${process.env.REACT_APP_API_PORT}/api/wrm`

export const  fetchWRMParamChartData = async (deviceId, fromTime, toTime) => {
  try {
    const url = `${API_BASE_URL}/getWrmChart`;
    const params = {
...
export const fetchAnomalyChartData = async (deviceId, fromTime, toTime) => {
  try {
      const url = `${API_BASE_URL}/getWrmChartOutlier`;
      const params = {
        param_device_id: deviceId,
        param_start_date: `${fromTime}`,  // Add single quotes around the dates
        param_end_date: `${toTime}`,
...
export const fetchOutlierChartData = async (deviceId, fromTime, toTime) => {
  try {
      const url = `${API_BASE_URL}/getWrmChartAnomaly`;
      const params = {
        param_device_id: deviceId,
        param_start_date: `${fromTime}`,  // Add single quotes around the dates
        param_end_date: `${toTime}`,
